...
/**
 * An API module of Bul Money contracts.
...
export class BombFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    this.BUL = new ERC20(deployments.Bul.address, provider, 'BUL');
    this.BSHARE = new ERC20(deployments.BShare.address, provider, 'BSHARE');
    this.BULBOND = new ERC20(deployments.BULBond.address, provider, 'BULBOND');
    ...
    // Uniswap V2 Pair
    this.BOMBBTCB_LP = new Contract(externalTokens['BUL-BTCB-LP'][0], IUniswapV2PairABI, provider);
  ...
  unlockWallet(provider: any, account: string) {
    ...
    const tokens = [this.BUL, this.BSHARE, this.BULBOND, ...Object.values(this.externalTokens)];
  ...
  async getBombStat(): Promise<TokenStat> {
    ...
    const supply = await this.BUL.totalSupply();
    const bombRewardPoolSupply = await this.BUL.balanceOf(BombGenesisRewardPool.address);
    const bombRewardPoolSupply2 = await this.BUL.balanceOf(BombRewardPool.address);
    const bombCirculatingSupply = supply.sub(bombRewardPoolSupply).sub(bombRewardPoolSupply2);
    //  const priceInBNB = await this.getTokenPriceFromPancakeswap(this.BUL);
    //const priceInBNBstring = priceInBNB.toString();
    const priceInBTC = await this.getTokenPriceFromPancakeswapBTC(this.BUL);
    ...
    return {
      ...
      totalSupply: getDisplayBalance(supply, this.BUL.decimal, 0),
      circulatingSupply: getDisplayBalance(bombCirculatingSupply, this.BUL.decimal, 0),
    };
  }
  ...
  async getLPStat(name: string): Promise<LPStat> {
    ...
    const token0 = name.startsWith('BUL') ? this.BUL : this.BSHARE;
    const isBomb = name.startsWith('BUL');
  ...
  async getBondStat(): Promise<TokenStat> {
    ...
    const priceOfBULBondInDollars = (Number(bombStat.priceInDollars) * modifier).toFixed(4);
    const supply = await this.BULBOND.displayedTotalSupply();
    return {
      tokenInFtm: bondPriceInBNB,
      priceInDollars: priceOfBULBondInDollars,
      totalSupply: supply,
  ...
  async getBombStatInEstimatedTWAP(): Promise<TokenStat> {
    ...
    const expectedPrice = await Oracle.twap(this.BUL.address, ethers.utils.parseEther('10000'));

    const supply = await this.BUL.totalSupply();
    const bombRewardPoolSupply = await this.BUL.balanceOf(BombRewardPool.address);
  ...
  async getPoolAPRs(bank: Bank): Promise<PoolStats> {
    ...
    const stat = bank.earnTokenName === 'BUL' ? await this.getBombStat() : await this.getShareStat();
  ...
  async getTokenPerSecond(
    ...
    const rewardPerSecond = await poolContract.tSharePerSecond();
    if (depositTokenName.startsWith('BUL')) {
  ...
  async getDepositTokenPriceInDollars(tokenName: string, token: ERC20) {
    ...
    if (tokenName === 'WBNB') {
      ...
      if (tokenName === 'BUL-BTCB-LP') {
        tokenPrice = await this.getLPTokenPrice(token, this.BUL, true);
      ...
      } else if (tokenName === 'BUL-BTCB-APELP') {
        tokenPrice = await this.getApeLPTokenPrice(token, this.BUL, true);
      } else {
  ...
  async getTotalValueLocked(): Promise<Number> {
    ...
    const bombStakeBalanceOf = await this.BUL.balanceOf(this.XBOMB.address);

    const boardroomTVL = Number(getDisplayBalance(boardroomtShareBalanceOf, this.BSHARE.decimal)) * Number(BSHAREPrice);
    const bombTVL = Number(getDisplayBalance(bombStakeBalanceOf, this.BUL.decimal)) * Number(BOMBPrice);
  ...
  async getTokenPriceFromPancakeswapBOMBUSD(): Promise<string> {
    ...
    //  const wbnb = new Token(56, WBNB[0], WBNB[1]);
    const btcb = new Token(56, this.BTC.address, this.BTC.decimal, 'BTCB', 'BTCB');
    const token = new Token(56, this.BUL.address, this.BUL.decimal, this.BUL.symbol);
    try {
      const wftmToToken = await Fetcher.fetchPairData(btcb, token, this.provider);
      const priceInBUSD = new Route([wftmToToken], token);
      ...
      console.error(`Failed to fetch token price of ${this.BUL.symbol}: ${err}`);
    }
  }
  ...
  async getBombStakeAPR() {
    ...
    //Mgod formula
    const amountOfRewardsPerDay = epochRewardsPerShare * Number(BOMBPrice) * 4;
    const xBombBombBalanceOf = await this.BUL.balanceOf(this.XBOMB.address);
  ...
  async estimateZapIn(tokenName: string, lpName: string, amount: string): Promise<number[]> {
    ...
    if (tokenName === BNB_TICKER) {
      ...
      const token = tokenName === BOMB_TICKER ? this.BUL : this.BSHARE;
  ...
  async swapBULBondToBShare(bbondAmount: BigNumber): Promise<TransactionResponse> {
    const {BShareSwapper} = this.contracts;
    return await BShareSwapper.swapBULBondToBShare(bbondAmount);
  }
  ...
  async getBShareSwapperStat(address: string): Promise<BShareSwapperStat> {
    ...
    const bbondBalanceBN = await BShareSwapper.getBULBondBalance(address);
